...
//                    .foregroundColor(.black)
        ...
        .onAppear{
            if docListner == nil{
                ...
                docListner = Firestore.firestore().collection("Cultural Routes").document(postID).addSnapshotListener({
                    snapshot, error in
                    if let snapshot{
                        if snapshot.exists{
    ...
    func deletePost(){
        Task{
            ...
            do{
                guard let postID = post.id else{return}
                try await Firestore.firestore().collection("Cultural Routes").document(postID).delete()
            }catch{
